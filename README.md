...
基于大语言模型的智能知识库系统，灵感来自 Skybase 等企业级 AI 知识库的核心能力，覆盖知识采集、知识消费、开放集成、应用封装四大层次。
...
基于 Skybase 产品能力拆解，按「知识消费 → 知识采集 → 开放集成 → 应用封装」分层，结合项目现状排列优先级：
...
| **用户 & 权限** | Skybase 官网（工作台） | 注册登录、JWT 鉴权、角色权限、知识库归属 | ✅ 已有 |

### P1 — 优先做（显著提升体验 & 覆盖 Skybase 核心差异化能力）
...
| **管理员仪表盘** | Skybase 官网（部分） | 用户管理、统计报表页 | ⚠️ 不决定产品价值，后补即可 |
...
> - **P1** 是 Skybase 的差异化竞争力——智能编译 vs 单纯存储、代码仓库问答、跨会话记忆、检索精度，不做等于只做了半个产品
